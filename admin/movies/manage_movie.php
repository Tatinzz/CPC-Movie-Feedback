    ...
    <div class="container-fluid">
        <div class="card card-outline card-info rounded-0 shadow">
            ...
            <div class="card-body rounded-0">
                ...
                <form action="" id="movie-form" enctype="multipart/form-data">
     ...
     function displayImg(input, _this) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                // Update the image ID to "img#cimg"
                $('#img#cimg').attr('src', e.target.result);
            }

            reader.readAsDataURL(input.files[0]);
        } else {
            // Update the image ID to "img#cimg"
            $('#img#cimg').attr('src', "<?php echo validate_image(isset($image_path) ? $image_path : '') ?>");
        }
    }
